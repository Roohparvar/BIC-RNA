def parse_rna_pseudoknot(structure, sequence, output_file):
    open_stack = []
        ...
        if structure[i] == '[':
            ...
            while close_count > 0 and open_stack:
                ...
                if open_count == close_count:
                    knot_sequence = open_seq + paired_seq[:close_count]
                    output_file.write(f"{knot_sequence}\n")
                    close_count = 0

                elif open_count > close_count:
                    knot_sequence = open_seq[-close_count:] + paired_seq[:close_count]
                    output_file.write(f"{knot_sequence}\n")
                    open_stack.append((f"S{open_count - close_count}", open_seq[:-close_count]))
                    close_count = 0

                elif open_count < close_count:
                    knot_sequence = open_seq + paired_seq[:open_count]
                    output_file.write(f"{knot_sequence}\n")
